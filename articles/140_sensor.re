...
Grove システムに対応するセンサーであれば、ケーブルを差し込むだけで安全に接続できるように設計されています。

アナログ出力のセンサーは 0〜3.3V の範囲で信号を出します。
Arduino 側ではこの電圧をそのまま扱うことができ、@<code>{analogRead()} を使って電圧値を数値として取得できます。
追加の回路や部品を用意する必要はありません。

一方で、Raspberry Pi の GPIO ピンも 3.3V 専用ですが、アナログ入力機能を持っていません。
そのため、水位センサーや温度センサーなどのアナログ信号を扱う場合には、別途 A/D 変換IC を配線して接続する必要があります。
信号の種類や電圧を意識せず扱える点で、Arduino系の方がセンサー接続には適しています。
...
次のスケッチ@<fn>{sensor_monitor}をArduino IDEで開いて書き込みます。
各センサーが接続されていない場合でも、エラーを出さずに動作します。

//footnote[sensor_monitor][Arduinoのプラグラムのこと、拡張子は.inoで言語はC++がベース]
...
ただし、接続場所はI2Cポートです。
照度と騒音ではI2Cポートをすでに二つ使用しているため、どちらかを割り当てる必要があります。
筆者は水耕栽培でポンプやエアレーションを使うため、それらに異常があったときに検知用に騒音センサーを使います。
そういった必要性を感じない方は、騒音センサーを外して、空いたコネクタにLCD液晶ディスプレイを接続すると良いでしょう。

//TODO 画像

このようにLCDにセンサー値が順次更新表示されます。
一度ボードにプログラムは書き込みされると、あとはPCに接続せずに、電力を供給し続けるだけでセンサー値を確認することができます。
