...
多くのセンサーは3.3 Vまたは5Vで動作し、出力も0〜5Vのアナログ信号や単純なデジタル信号です。
Arduinoでは、これらの電圧をそのまま安全に扱うことができます。
A0〜A5のピンにアナログセンサーを挿せば、その電圧をanalogRead()で数値として取得できます。

一方で、Raspberry PiのGPIOピンは3.3 V専用です。
5V信号をそのまま入力すると壊れるおそれがあるため、「レベルシフタ」や「抵抗による分圧回路」などを追加する必要があります。
また、Raspberry Piにはアナログ入力ピンがありません。
水位センサーや温度センサーのようにアナログ出力を持つデバイスを使うには、別途A/D変換ICを配線して接続しなければなりません。文系には訳が分かりませんね。
...
この方法だと前述の通り、初心者にとって、配線ミスが多発しやすく、取り回しも悪く、接触不良も起きやすくなります。
...
また、Seeed Studio社の公式サイトでは、Groveコネクタ対応のセンサーモジュールを**300種類以上**販売しています。
...
これはマイコンの配線に悩まされた人間にとって、福音以外の何者でもあありません。
本章では、このGroveコネクタを12個搭載したSeeeduino Lotusを最大限活用した、センサー接続とデータ取得の方法を解説していきます。
...
前置きが長くなりましたが、実際にSeeeduino Lotusをセットアップし、センサーからデータを取得するまでの手順を解説します。
...
Seeeduino Lotus バージョンに注意@<fn>{seeeduino_lotus}	1つ	Seeed Studio	¥10,800
 ...
 * 「追加のボードマネージャのURL」欄に次を入力
    @<href>{https://files.seeedstudio.com/arduino/package_seeeduino_boards_index.json}
 * ［OK］を押して閉じる
...
次のスケッチ（Arduinoのプラグラム）をArduino IDEで開いて書き込みます。
