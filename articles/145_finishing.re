...
@<chap>{140_sensor} @<title>{140_sensor} の章では、センサーの値を取得することができました。

ここまでのまとめとして、Seeduinoから取得したデータもまとめてThingSpeakに送信し、Grafanaでダッシュボードを作成する手順を紹介します。
...
@<chap>{120_dashboard} @<title>{120_dashboard} の章でThingSpeakのアカウントを作成し、チャンネルを作成しました。
そこに新たにフィールドを追加します。
...
ThingSpeakでデータが受信できているのを確認できたら、最後に、Grafanaのダッシュボードを追加します。

@<chap>{120_dashboard} @<title>{120_dashboard} の章でGrafanaの設定をしていれば、データソースとしてThingSpeakがすでに設定されているはずなので、データソースの設定は不要です。
